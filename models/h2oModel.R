...
train_mens_h2o = feature_matrix_wide_mens %>%
  select(-Match_Id, -Tournament, -Tournament_Date, -Round_Description) %>%
...
mens_submission <- bind_rows(mens_submission, steve_darcis1)
...
womens_submission <- bind_rows(womens_submission, Clara_Burel)

# Export to CSV - this is the submission file!
mens_submission %>% write_csv("submission/datathon_submission_mens_495.csv")
womens_submission %>% write_csv("submission/datathon_submission_womens_495.csv")
